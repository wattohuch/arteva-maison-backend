...
const templates = require('../services/whatsappTemplates');

const { TEMPLATE_CONTRACTS } = templates;
...
// @access  Private/Admin
//
// The work itself lives in services/whatsappTemplates so the same code runs
// from here and from the boot-time flag, which is how this gets done when
// nobody can hold an admin session and the Meta account at the same time.
//
// Body (all optional): languages, only, dryRun.
const provisionTemplates = asyncHandler(async (req, res) => {
    const report = await templates.provision({
    ...
    if (report.error) {
        return res.status(report.permanent ? 422 : 502).json({
            success: false,
    ...
    res.json({
        ...
        data: {
            ...report,
            nextStep: report.dryRun
                ? 'Nothing was submitted. Re-send without dryRun to file these for review.'
                : 'Meta reviews these, usually within minutes. Set the environment variables above once they read APPROVED in GET /api/whatsapp/templates.',
        },
    });
});
